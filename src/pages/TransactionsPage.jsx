...
const STATUS_MAP = {
  ...
  refunded: { label: 'Reembolsado',class: 'bg-zinc-400/10 text-th-text-3 border-zinc-400/20' },
}

function StatusBadge({ status }) {
  const s = STATUS_MAP[status] ?? { label: status, class: 'bg-zinc-400/10 text-th-text-3 border-zinc-400/20' }
  return (
...
function DetailsModal({ sale, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
      <div className="w-full max-w-md bg-th-surface border border-th-line rounded-2xl shadow-2xl">

        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-th-line">
          <div>
            <h2 className="font-semibold text-th-text text-sm">Detalhes da transação</h2>
            <p className="text-xs text-zinc-500 mt-0.5">{formatDate(sale.created_at)}</p>
          </div>
          <button
            onClick={onClose}
            className="text-zinc-500 hover:text-th-text-2 transition-colors p-1 rounded-lg hover:bg-th-raised"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-th-raised/60 rounded-xl px-4 py-3">
            ...
            <div className="bg-th-raised/60 rounded-xl px-4 py-3 col-span-2">
              <p className="text-xs text-zinc-500 mb-1">Cliente</p>
              <p className="text-sm text-th-text-2 font-medium">{sale.customer_name}</p>
          ...
          {sale.qr_code_url ? (
            ...
            <div className="flex flex-col items-center gap-2 py-4 text-th-text-3">
          ...
          {sale.qr_code_text && (
            <div>
              ...
              <div className="bg-th-raised border border-zinc-300 dark:border-zinc-800 rounded-xl px-4 py-3 mb-3">
                <p className="text-xs text-th-text-3 font-mono break-all leading-relaxed select-all">
                  {sale.qr_code_text}
                </p>
              </div>
              <button
...
export default function TransactionsPage() {
  ...
  const fetchSales = useCallback(async ({ silent = false } = {}) => {
    const key = `sales:${user.id}`
    const cached = cacheGet(key)

    if (cached && !silent) {
      // Cache hit: exibe imediatamente sem skeleton + re-fetch em background
      setSales(cached)
      setLoading(false)
  ...
  return (
    <DashboardLayout>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-th-text">Transações</h1>
          <p className="text-zinc-500 text-sm mt-1">Histórico de vendas geradas.</p>
        </div>
        {sales.length > 0 && (
          <span className="text-xs font-medium text-zinc-500 bg-th-raised border border-zinc-300 dark:border-zinc-800 px-3 py-1.5 rounded-full">
      ...
      {loading ? (
        <div className="th-card bg-th-surface border border-th-line rounded-xl overflow-hidden">
          <div className="hidden md:grid grid-cols-[160px_1fr_120px_110px_80px] gap-4 px-6 py-3 border-b border-th-line text-xs font-medium text-zinc-500 uppercase tracking-wider">
            <span>Data</span>
            <span>Cliente</span>
            <span>Valor</span>
            <span>Status</span>
          ...
          <div className="th-card bg-th-surface border border-th-line rounded-xl overflow-hidden">
            ...
            <ul className="divide-y divide-th-line">
              {paginated.map(sale => (
                <li
                  key={sale.id}
                  className="grid md:grid-cols-[160px_1fr_120px_110px_80px] gap-4 items-center px-6 py-4 hover:bg-th-raised/40 transition-colors"
                >
                  <p className="text-xs text-zinc-500 tabular-nums">{formatDate(sale.created_at)}</p>
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-th-text truncate">{sale.customer_name}</p>
  ...
  return (
    <div className="bg-th-surface border border-th-line rounded-xl p-14 text-center">
      <div className="w-16 h-16 bg-th-raised rounded-2xl flex items-center justify-center mx-auto mb-4">
        <ReceiptText className="w-8 h-8 text-th-text-4" />
      </div>
      <h3 className="text-th-text-2 font-medium mb-2">Nenhuma transação ainda</h3>
      <p className="text-th-text-4 text-sm max-w-xs mx-auto">
